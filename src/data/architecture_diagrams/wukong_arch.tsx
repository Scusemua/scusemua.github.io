...
const WukongArchitecture = (props: WukongArchitectureProps) => {
    ...
    const [popoverMaxWidth, setPopoverMaxWidth] = React.useState<string>(getDefaultPopoverWidth(mq_xl, mq_lg, mq_md, mq_sm));

    const [popoverContent, setPopoverContent] = React.useState<React.JSX.Element | string>('');
    ...
    const onClickClients = (event: MouseEvent) => {
        ...
        setPopoverContent("Wukong users submit a Python computing job to the DAG generator, which uses the Dask " +
            "library to convert the job into a DAG.");
        setPopoverAnchorEl({getBoundingClientRect, nodeType: 1});
    }

    const onClickDAG = (event: MouseEvent) => {
        event.stopPropagation();
        ...
        setPopoverMaxWidth(getDefaultPopoverWidth(mq_xl, mq_lg, mq_md, mq_sm));
        setPopoverOpen(true);
        setPopoverHeader("Directed Acyclic Graphs (DAGs)");

        const content = (
            <Stack direction={'column'}>
                <Typography sx={{p: 1}}>
                    {"Some types of compute- and data-intensive applications are inherently parallelizable and " +
                        "can be structured as a directed acyclic graph (DAG) of short, fine-grained tasks."}
                </Typography>
                <Typography sx={{p: 1}}>
                    {"The nodes of the DAG represent the fine-grained tasks, while the edges between the nodes " +
            ...
            <Stack direction={'column'}>
                <Typography sx={{p: 1}}>
                    {"The Static Schedule Generator generates static schedules from the DAG. For a DAG with 𝑛 " +
                        ...
                        "of the task nodes that are reachable from L and all of the edges into and out of these nodes."}
                </Typography>
                <Typography sx={{p: 1}}>
                    {"The data for a task node includes the task’s code and the KVS keys for the task’s input data. " +
                        "The schedule for L is easily computed using a depth-first search (DFS) that starts at L."}
                </Typography>
                <div style={{
    ...
    const onClickInitialInvokers = (event: MouseEvent) => {
        ...
        setPopoverMaxWidth(getDefaultPopoverWidth(mq_xl, mq_lg, mq_md, mq_sm));
        setPopoverOpen(true);
        setPopoverHeader("Initial Task Executor Invokers");
        setPopoverContent("Wukong workflow execution starts when the static scheduler’s Initial-Executor Invokers " +
            "assign each static schedule produced by the Static-Schedule Generator to a separate Executor.");
        setPopoverAnchorEl({getBoundingClientRect, nodeType: 1});
    }

    const onClickDagGenerator = (event: MouseEvent) => {
        event.stopPropagation();
        ...
        setPopoverMaxWidth(getDefaultPopoverWidth(mq_xl, mq_lg, mq_md, mq_sm));
        setPopoverOpen(true);
        setPopoverHeader("DAG Generator");
        setPopoverContent("The DAG generator uses the Dask Python library to convert the job into a DAG.");
        setPopoverAnchorEl({getBoundingClientRect, nodeType: 1});
    }

    const onClickSubscriberProcess = (event: MouseEvent) => {
        event.stopPropagation();
        ...
        setPopoverMaxWidth(getDefaultPopoverWidth(mq_xl, mq_lg, mq_md, mq_sm));
        setPopoverOpen(true);
        setPopoverHeader("Subscriber Process");
        setPopoverContent("The subscriber process is a simple, dedicated process that listens for certain important " +
            "messages. Specifically, the subscriber process receives notifications that the result of a DAG " +
    ...
    return (
        ...
        >
            ...
            <path
                ...
                d="M90.82 69.336c6.528 6.656 6.528 17.445 0 24.102a16.49 16.49 0 0 1-23.64 0c-6.528-6.657-6.528-17.446 0-24.102a16.49 16.49 0 0 1 23.64 0"
            />
            <text xmlSpace="preserve" fontFamily="DejaVuSans" fontSize={24} onClick={onClickClients}>
            ...
            <path onClick={onClickDagGenerator} fill="#fff" d="M417 63.25h136v68H417Z"/>
            <path
                onClick={onClickDagGenerator}
                fill="none"
                stroke="#000"
                strokeLinecap="round"
                strokeLinejoin="round"
            ...
            <text
                ...
                fontWeight="bold"
            >
                <tspan onClick={onClickDagGenerator} x="463.082 478.00599 492.43598 506.86598" y={90.522}>
                    {"DAG "}
                </tspan>
                <tspan
                    onClick={onClickDagGenerator}
            ...
            <path onClick={onClickDAG} fill="#fff" d="M618 63.25h136v148.5H618Z"/>
            <path
                onClick={onClickDAG}
                fill="none"
                stroke="#000"
            ...
            <path onClick={onClickInitialInvokers} fill="#fff" d="M417 252h337v112H417Z"/>
            <path
                onClick={onClickInitialInvokers}
                fill="none"
                stroke="#000"
                strokeLinecap="round"
                strokeLinejoin="round"
            ...
            <path onClick={onClickSubscriberProcess} fill="#fff" d="M230 296h136v68H230Z"/>
            <path
                onClick={onClickSubscriberProcess}
                fill="none"
                stroke="#000"
                strokeLinecap="round"
                strokeLinejoin="round"
            ...
            />
            <path onClick={onClickClients} d="m225.352 97.148-9.497-3.574-.011 7.125Zm0 0"/>
            <path
                onClick={onClickClients}
                fill="none"
                stroke="#000"
            ...
            <text xmlSpace="preserve" fontFamily="DejaVuSans" fontSize={24} onClick={onClickClients}>
                ...
                <tspan onClick={onClickClients} x="139.30818 144.63617 158.38817 172.62018" y={120.25}>
                    {"jobs"}
                </tspan>
            </text>
            <path
            ...
            />
            <path onClick={onClickScheduleGenerator} fill="#fff" d="M417 152h136v68H417Z"/>
            <path
                onClick={onClickScheduleGenerator}
                fill="none"
                stroke="#000"
                strokeLinecap="round"
                strokeLinejoin="round"
            ...
            >
                <tspan
                    ...
                    y={30.136}
                >
                    {"Interfaces & static scheduler (\xA7 3.2 + 3.5 of Paper)"}
                </tspan>
            </text>
            <path
                fill="none"
            ...
            <Popover
                ...
                onClose={handleClose}
                disableAutoFocus
                style={{width: popoverMaxWidth, maxWidth: popoverMaxWidth}}
            >
                <Paper>
                    <Stack>
                        <Typography align={'center'} sx={{p: 1, fontSize: "24px"}}>{popoverHeader}</Typography>
                        <Item>
                            {isString(popoverContent) ?
                                <Typography sx={{p: 1}}>{popoverContent}</Typography> : popoverContent}
                        </Item>
                        <Item>
